...
public class _944_Delete_Columns_to_Make_Sorted {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Removed Columns Amount: " + minDeletionSize(strs));
    }
    ...
    public static int minDeletionSize(String[] strs) {
        ...
        for (int col = 0; col < strs[0].length(); col++) {    // given that all string are equal length
            for (int row = 0; row < strs.length - 1; row++) {

                char curr = strs[row].charAt(col);
                char next = strs[row+1].charAt(col);
